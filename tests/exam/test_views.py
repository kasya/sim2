...
class ExamViewTestCase(TestCase):
    ...
    def test_api_exam_list_get(self):
        ...
        self.assertEqual(
            ExamSerializer(
                Exam.objects.filter(subject=self.exam.subject), many=True
            ).data,
            response.data,
        )
    ...
    def test_exam_intro_get_unauthenticated(self):
        ...
        next = reverse(
            "exam_intro",
            kwargs={"exam_id": self.exam.id, "exam_mode": "practice"},
        )
        self.assertRedirects(response, f'{reverse("login")}?next={next}')
    ...
    def test_practice_intro_get(self):
        ...
        self.assertEqual(
            ExamAttempt.PRACTICE_MODE, response.context["exam_mode"]
        )
        self.assertInHTML(
            "<p>You chose to practice "
            f"{self.subject.name} {self.exam.name}.</p>",
    ...
    def test_exam_intro_get(self):
        ...
        self.assertInHTML(
            f"<p>You chose {self.exam.subject.name} "
            f"{self.exam.name} as your exam.</p>",
            response.content.decode("utf-8"),
        )

    def test_subject_attempt_intro_get(self):
        """
        Check that intro page for a given subject
        exam attempt renders correctly.
        """
        ...
        self.assertEqual("exam", response.context["exam_mode"])
        self.assertInHTML(
            f"<p>You chose {self.subject.name} as your exam.</p>",
            response.content.decode("utf-8"),
        )
    ...
    def test_exam_intro_post_extra_time(self):
        ...
        exam_attempt = ExamAttempt.objects.filter(
            user=self.user, exams=self.exam
        ).last()
        self.assertEqual(
            exam_attempt.duration_minutes,
    ...
    def test_subject_attempt_practice_mode_intro_post(self):
        ...
        exam_attempt = ExamAttempt.objects.filter(
            user=self.user, mode=ExamAttempt.PRACTICE_MODE
        ).last()
        self.assertEqual(
            exam_attempt.duration_minutes,
            self.exam.duration_minutes + self.user.required_extra_time,
        ...
        self.assertRedirects(
            ...
            reverse(
                "exam_page",
                kwargs={
                    "attempt_id": ExamAttempt.objects.filter(user=self.user)
                    .last()
                    .id
                },
            ),
    ...
    def test_subject_attempt_exam_mode_intro_post(self):
        ...
        exam_attempt = ExamAttempt.objects.filter(
            user=self.user, mode=ExamAttempt.EXAM_MODE
        ).last()
        ...
        self.assertRedirects(
            ...
            reverse(
                "exam_page",
                kwargs={
                    "attempt_id": ExamAttempt.objects.filter(
                        user=self.user, exams=self.exam
                    )
                    .last()
                    .id
    ...
    def test_exam_attempt_practice_mode_intro_post(self):
        ...
        self.assertEqual(
            exam_attempt.questions.count(), self.exam.questions.count()
        )
    ...
    def test_exam_attempt_finish_get(self):
        ...
        answer_attempt = AnswerAttempt.objects.create(
            attempt=self.exam_attempt, question=self.question
        )
    ...
    def test_exam_attempt_practice_mode_finish_get_context(self):
        ...
        self.assertEqual(
            ...
            f"Congratulations! You've finished {ExamAttempt.PRACTICE_MODE} "
            f"in {self.subject.name} {self.exam.name}! "
            f"Your grade is {self.exam_attempt.grade}%.",
        )
    ...
    def test_subject_attempt_exam_mode_finish_get_context(self):
        ...
        self.client.login(username=self.user.username, password=self.password)
        answer_attempt = AnswerAttempt.objects.create(
            attempt=self.subject_attempt, question=self.question
        )
        answer_attempt.answers.add(self.answer)
        answer_attempt.save()
        ...
        response = self.client.get(
            reverse(
                "exam_finish", kwargs={"attempt_id": self.subject_attempt.id}
            )
        )
        self.assertEqual(response.context["grade"], self.subject_attempt.grade)
        self.assertEqual(
            response.context["status"],
            f"Congratulations! You've finished {ExamAttempt.EXAM_MODE} in "
            f"{self.subject.name} {self.exam.name}! "
            f"Your grade is {self.subject_attempt.grade}%.",
        )
    ...
    def test_get_attempt_unauthenticated(self):
        ...
        response = self.client.get(
            reverse("get_attempt", kwargs={"attempt_id": 1})
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_get_attempt_no_attempt(self):
        ...
        self.client.login(username=self.user.username, password=self.password)
        response = self.client.get(
            reverse("get_attempt", kwargs={"attempt_id": 10})
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_get_attempt(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data, ExamAttemptSerializer(self.exam_attempt).data
        )
