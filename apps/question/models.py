...
class Answer(models.Model):
    ...
    def __str__(self):
        return _("<Answer>: {text}, id# {id}").format(
...
class Question(models.Model):
    ...
    correct_answers = models.ManyToManyField(
        Answer, related_name="correct_answers"
    )
    wrong_answers = models.ManyToManyField(
        Answer, related_name="wrong_answers"
    )

    category = models.ForeignKey("QuestionCategory", on_delete=models.CASCADE)
    exam = models.ForeignKey(
        "exam.Exam", related_name="questions", on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return _("<Question>: {text}, id# {id}:").format(
            text=self.text, id=self.id
        )
...
class QuestionCategory(models.Model):
    ...
    def __str__(self):
        return _("<Category>: {name}, id# {id}").format(
